importando fonte

-- colocar o @nome da fonte escolhida 

![image](https://github.com/user-attachments/assets/79666c76-3255-4d01-ae92-99ed1cb75e56)




-- declarar todas as fontes 

![image](https://github.com/user-attachments/assets/7036e193-07ff-42d6-a0f9-f4aae33eebf4)



link do vídep 

https://youtu.be/3YIXnxA1kqg?si=gC9dEqA6Pyqi0COy



----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


Quando passar o mouse mudar a cor 

}
 background-color: orange;
}


--- SOBRE A FONTE 

COLOCAR CADA UMA INDIVIDUALMENTE NO CSS SEM A PASTA GILROY 
UTIÇIZAR O FONTE FACE 


@font-face {
    font-family: 'Gilroy-Black';
    src:url('Gilroy-BlackItalic.ttf');
    
}

@font-face {
    font-family: 'Gilroy-BlackItalic';
    src:url('Gilroy-BlackItalic.ttf'),    
}



.navbar-links a {
    color: white;
    text-decoration: none;
    font-size: 20px;
    transition: color 0.3s;
    font-family: 'Gilroy-BlackItalic'; /*ver como fazer isso*/
    
}

